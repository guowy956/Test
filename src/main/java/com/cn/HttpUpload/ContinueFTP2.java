...
public class ContinueFTP2 implements Runnable{
    ...
    public ContinueFTP2(String _ftpURL,String _username,String _pwd,String _ftpport,String _file1,String _file2 ){
        ftpURL = _ftpURL; username = _username;
        pwd = _pwd;       ftpport = _ftpport;
...
//    创建进度输出
    public static long createProcess(int c,InputStream in,long process,long localSize,long step,FileOutputStream out ,byte[] bytes) throws IOException {
...
//        判断远程文件是否存在
        FTPFile[] ftpFiles = ftpClient.listFiles(new String(remote.getBytes("GBK"),"iso-8859-1"));
...
//         进行断点续传，并记录状态
            ...
            int c = 0;
            createProcess(c, in, process, localSize, step, out ,bytes);
            in.close();
            out.close();
...
//        检查远程是否存在文件
        FTPFile[] ftpFiles = ftpClient.listFiles(new String(remoteFileName.getBytes("GBK"),"iso-8859-1"));
        if(ftpFiles.length==1){
...
//        显示进度的上传
    ...
    public UploadStatus CreateDirecroty(String remote,FTPClient ftpClient) throws IOException{
        ...
        String directory = remote.substring(remote.lastIndexOf("/")+1);
        if(!directory.equalsIgnoreCase("/")&&!ftpClient.changeWorkingDirectory(new String(remote.getBytes("GBK"),"iso-8859-1"))){
            int start = 0;
            int end = 0;
