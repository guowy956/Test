...
//    有参构造函数，先构造需要的数据
    public MultiTheradDownLoad(String filepath ,int threadNum){
        ...
        startos = new long[this.threadNum];
...
//        在请求url内获取文件资源的名称；此处没考虑文件名为空的情况，此种情况可能需使用UUID来生成一个唯一数来代表文件名。
        ...
        try {
            ...
            threadLength = fileLength/threadNum; // 每个线程需要下载资源的大小
            ...
            if(file.exists() && file.length()==fileLength){
                ...
                //开始断点设置方法
                setBreakPoint(startos,endPos,tmpfile);
                ExecutorService exec = Executors.newCachedThreadPool();
                for (int i = 0;i<threadNum;i++){
//                    实现下载功能的内部类，通过读取断点来设置向服务器请求的数据区间
                    exec.execute(new DownLoadThread(startos[i],endPos[i],this,i,tmpfile,latch));
                }
...
//                  防止网络阻塞，设置指定的超时时间；单位都是ms。超过指定时间，就会抛出异常
                    ...
                    httpcon.setConnectTimeout(200000);//链接的超时设置
